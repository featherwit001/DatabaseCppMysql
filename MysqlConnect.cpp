...
bool DataBase::Connect(const char* ip, const char* name, const char* cypher, const char* database_name, const int port)
{
    ...
    if ((mysql_real_connect(_mysql, ip, name, cypher, database_name, port, NULL, CLIENT_MULTI_STATEMENTS))==NULL) //中间分别是主机，用户名，密码，数据库名，端口号（可以写默认0或者3306等），可以先写成参数再传进去  
    {
...
bool DataBase::Implement(const char* sentence)
{
    ...
    if (!(_res = mysql_store_result(_mysql)))   //获得sql语句结束后返回的结果集  
    {
        ...
        if (mysql_error(_mysql))
        {
            ...
            printf("Couldn't get result for %s\n", mysql_error(_mysql));
